...
export async function getPolicies(status?: string, options?: { chainId?: number }): Promise<EndpointPolicyDTO[]> {
  return withAuthRead((auth) => _getPolicies(auth.userId, status, options));
}

export async function getPolicy(policyId: string): Promise<EndpointPolicyDTO> {
  return withAuthRead(async (auth) => {
    const policy = await _getPolicy(policyId, auth.userId);
...
export async function updatePolicy(
  ...
  return withAuth(async (auth) => {
    const existing = await _getPolicy(policyId, auth.userId);
